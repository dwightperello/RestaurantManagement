...
class EditMenuUseCase @Inject constructor(private val repository: NetworkRepository) {

    operator fun invoke(): Flow<ResultState<ArrayList<AllMenuModelItem>>> = flow {
        try {
            emit(ResultState.Loading)
            val response = repository.Menus()
            emit(ResultState.Success<ArrayList<AllMenuModelItem>>(response))
